...
export class KotlinLanguageClient {
    ...
    public async start(config: BrexKotlinLanguageServerConfig, options: { outputChannel: vscode.OutputChannel }): Promise<void> {
        ...
        if (!javaHome) {
            vscode.window.showErrorMessage(`Could not find Java ${config.jvmTarget} installation. Please set brex.kotlinLanguageServer.javaHome in your settings to the path of the Java installation to proceed.`);
